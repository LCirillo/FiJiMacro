...
run("Clear Results");
print("Cell" + " \t " + "NEBD" + " \t " + "Metaphase" + " \t " + "Anaphase" + " \t " + "NEBD_to_Meta" + " \t " + "NEBD_to_Ana");
...
for (MitoCell = 0; MitoCell < MitoticCells; MitoCell ++) {
	...
	Dialog.createNonBlocking("MitoticTiming");
  	...
  	Dialog.show();
  	NEBD = (Dialog.getNumber())*FrameTime;
  	Meta = (Dialog.getNumber())*FrameTime;
  	Ana = (Dialog.getNumber())*FrameTime;
  	NEBD_to_Meta = Meta - NEBD;
  	NEBD_to_Ana = Ana - NEBD;
  	if (SavingOption == true){
  		saveAs("Tiff", Quantification + title + "_Cell_" + MitoCell);}
  	print("Cell_" + MitoCell + " \t " + NEBD + " \t " + Meta + " \t " + Ana + " \t " + NEBD_to_Meta + " \t " + NEBD_to_Ana);
  	if(isOpen("ToClose")){
